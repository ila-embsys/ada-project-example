...
.PHONY : all
all : init-repo zephyr-export build
...
.PHONY: configure
configure:
	west build -b $(BOARD) -p auto --cmake-only -- $(PLATFORM_SPECIFIC_LOCS_ARG) -DCONF_FILE=$(CONF_FILE) -DEXTRA_CFLAGS=$(EXTRA_CFLAGS)

.PHONY: build
build:
	west build -b $(BOARD) -p auto -- $(PLATFORM_SPECIFIC_LOCS_ARG) -DCONF_FILE=$(CONF_FILE) -DEXTRA_CFLAGS=$(EXTRA_CFLAGS)
